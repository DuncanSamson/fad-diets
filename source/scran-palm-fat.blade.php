---
title: Palm Fat
description: Fat extracted from the fruit of the oil palm tree, used widely in processed foods and cosmetics.
image: mars-bar.jpg
author: Duncan Samson
---
@extends('_layouts.article')
@section('content')
    <h1 class="text-7xl font-bold text-center mb-6 pb-3 mb-10 border-b border-b-emerald-200">Palm fat</h1>
    <h2 class="text-xl pt-8 pb-2 font-bold">How Is It Made?</h2>
    <p class="text-lg">Palm fruits are harvested and then steamed
        They are then pressed to remove the crude oil
        The oil is then clarified to remove impurities
        It is then fractionated by cooling
        This process separates the solid (palm fat) from the liquid fraction
        Chemical or physical refining may follow
        The fat is then filtered and dried
        Finally, it’s packaged for use in food products </p>
    <h2 class="text-xl pt-8 pb-2 font-bold">What Affects Could It Have On Your Brain?</h2>
    <p class="text-lg">Research on high-saturated fat diets indicates that diets rich in palm fat may alter brain function, potentially
        affecting memory and learning.
        <h2 class="text-xl pt-8 pb-2 font-bold">What Affects Could It Have On Your Body?</h2>
        <p class="text-lg" >Some studies suggest that a high intake of palm fat can contribute to weight gain and may increase cholesterol
        levels, posing a risk to heart health over time.</p>
    <h2 class="text-xl pt-8 pb-2 font-bold">Fun Facts</h2>
    <p class="text-lg">Palm fat is a key ingredient in many processed foods like biscuits and spreads.
        Palm fat is found in approximately 50% of all supermarket products in the UK</p>

    <h2 class="text-xl pt-8 pb-2 font-bold">Sources</h2>
    <ul class="list-disc pl-8">
        <li>
            <x-link href="https://www.gov.uk/government/publications/uk-consumption-of-sustainable-palm-oil-annual-review">UK
                consumption of sustainable palm oil: annual review.</x-link>
        </li>
        <li>
            <x-link href="https://ajcn.nutrition.org/article/S0002-9165%2823%2905094-3/fulltext">Palm oil and blood
                lipid–related markers of cardiovascular disease:</x-link>
        </li>
    </ul>
@endsection